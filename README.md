...
The documentation for the source code is generated by a shakefile and then copied into /docs.

To generate it yourself run:

`stack build; stack exec build-the-docs`

Then you can go to the docs directory and run as static server like `warp` or `http-server` from node

to see all the documentation from the current hash.
